...
const I18N = {
  fr: {
    ...
    entity_days_helper: "Entité texte unique contenant les jours actifs (ex: 1, 2, 3, 4, 5)",
  ...
  en: {
    ...
    entity_days_helper: 'Single text entity storing active days as comma-separated numbers 1-7 (Monday=1, e.g. 1, 2, 3, 4, 5)',
  ...
  de: {
    ...
    entity_days_helper: 'Text-Entität mit aktiven Tagen kommagetrennt (z.B. 1, 2, 3, 4, 5)',
  ...
  nl: {
    ...
    entity_days_helper: 'Tekst-entiteit met actieve dagen door komma\'s gescheiden (bijv. 1, 2, 3, 4, 5)',
  ...
  es: {
    ...
    entity_days_helper: 'Entidad de texto con días activos separados por comas (ej: 1, 2, 3, 4, 5)',
  ...
  it: {
    ...
    entity_days_helper: 'Entità di testo con giorni attivi separati da virgola (es: 1, 2, 3, 4, 5)',
  ...
  pl: {
    ...
    entity_days_helper: 'Encja tekstowa z aktywnymi dniami oddzielonymi przecinkami (np. 1, 2, 3, 4, 5)',
  ...
  pt: {
    ...
    entity_days_helper: 'Entidade de texto com os dias ativos separados por vírgulas (ex: 1, 2, 3, 4, 5)',
  ...
  ru: {
    ...
    entity_days_helper: 'Текстовый объект со списком активных дней через запятую (напр. 1, 2, 3, 4, 5)',
  ...
  sv: {
    ...
    entity_days_helper: 'Textentitet som sparar aktiva dagar separerade med kommatecken (t.ex. 1, 2, 3, 4, 5)',
...
class RetroAlarmCard extends HTMLElement {
  ...
  _isDayActive(dayIndex) {
    if (!this._config.entity_days || !this._hass?.states[this._config.entity_days]) return false;
    const raw = (this._hass.states[this._config.entity_days].state || '').trim();
    const tokens = raw.split(/[,\s]+/).map(s => s.trim()).filter(Boolean);
    return tokens.includes(String(dayIndex + 1));
  }
   ...
   */
  _toggleDay(dayIndex) {
    if (!this._hass || !this._config.entity_days) return;
    ...
    const newString = activeIndices.map(i => i + 1).join(', ');
  ...
  _updateDays() {
    ...
    for (let idx = 0; idx < 7; idx++) {
      const label = dayLabels[idx] || `J${idx + 1}`;
